...
Manager.prototype.garbageCollection = function () {
  ...
  while (i--) {
    ...
    if ('issued' in handshake && (now - handshake.issued) >= 3E4) {
      this.onDisconnect(ids[i], true);
    }
  }

  // clean up dead sockets
  // (based on https://github.com/natefaubion/socket.io/commit/25ee5ea7a9ba084e058818f9c6ce63456e8014f8)
  var self = this;
  ['closed', 'open', 'transports'].forEach(function (type) {
    ...
    while (i--) {
      if (!(ids[i] in self.handshaken)) {
        // call onDisconnect rather than deleting to ensure that pubsub channels are unsubscribed
        self.onDisconnect(ids[i], true);
      }
    }
  });
